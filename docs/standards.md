...
| **IETF RFC 4273 (BGP4-MIB)** | `bgpPeerTable` walk; peer state and address decode | BGP peer adjacency edges at precedence rank 7. |
| **IETF RFC 1850 (OSPF-MIB)** | `ospfNbrTable` walk; neighbour state decode | OSPF adjacency edges at precedence rank 6. |
...
| **IETF RFC 1213 (MIB-II)** | `sysName`, `sysDescr`, `sysUpTime`, `sysObjectID` from `system` group | Device identity and uptime. `sysName` is the canonical device identifier (normalized lowercase). |
...
| **SNMP v3 USM (RFC 3414)** | Only SNMPv2c community strings are currently supported. SNMPv3 is a planned addition. |
