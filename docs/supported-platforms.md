...
| Nokia | SR Linux | 26.3.2 | SNMP system group, LLDP (IEEE 802.1AB `1.0.8802.1.1.2`) | E2E containerlab (`make test-e2e-srl`) | **LLDP via SNMP is supported from 26.3.x** via the built-in SNMP framework (`lldp_mib.yaml` → `lldpLocPortTable` / `lldpRemTable`). Validated 2026-06 with `ghcr.io/nokia/srlinux:26.3.2`. **SR Linux 24.x does not expose LLDP-MIB** — returns `No Such Object` on every probe; LLDP there is gNMI/JSON-RPC only (issue #46). SNMP system group (`sysDescr`, `sysName`, `sysUpTime`, `sysObjectID`) works on all tested versions. |
...
**LLDP** (IEEE 802.1AB): Walker is specification-derived and has no per-vendor branching. Platforms that correctly implement the MIB at `1.0.8802.1.1.2` work without modification. Platforms confirmed to fail: SR Linux 24.x (gNMI only, #46). **SR Linux 26.3.x+ implements LLDP-MIB via the SNMP framework** — validated in `make test-e2e-srl`. Platforms with E2E containerlab coverage via synthetic SNMP agents: all platforms in `make test-e2e`.
