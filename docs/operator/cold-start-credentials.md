...
Bringing the topology exporter up against a fleet of devices for the first time is the moment most likely to lock devices out of their own monitoring. This runbook is the LD-12 operator guidance for getting through that window without tripping anyone's auth-lockout policy.
...
The LD-12 token-bucket trial limiter (`credentials.trial_rate_per_second`, default 5) caps the global trial rate so the exporter doesn't fire all those auth attempts simultaneously. The limiter slows the cold start, it doesn't prevent the lockout. Slowing the cold start is enough only if the candidate set per device is small.
