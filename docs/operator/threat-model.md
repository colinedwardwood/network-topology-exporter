...
|---|---|---|
| **Spoofing** — device side | An attacker on the management plane impersonates a target IP and returns crafted SNMP responses | SNMPv3 with auth + priv (operator choice); CIDR allow-list bounds blast radius; PDU parsing must not crash on hostile bytes (fuzz coverage — issue #70) |
...
| **Tampering** — device PDUs | A compromised or hostile device returns crafted PDUs designed to confuse the exporter | PDU fuzz coverage (#70); reconciliation surfaces protocol-conflict counters rather than silently merging; per-walker outcome counters surface walker-drift |
...
| **Information disclosure** — OTLP push | The configured OTLP receiver is captured or impersonated | TLS verification on the OTLP endpoint is operator-configurable (`output.otlp.tls_*`); operator must verify endpoint trust |
| **Denial of service** — device-induced crash | A crafted PDU panics the exporter; restart loop | Fuzz coverage (#70) is the primary mitigation. Until that lands, `restartPolicy: Always` on the container minimises operator-visible blast radius |
...
| **Denial of service** — self-DoS on targets | A misconfigured `discovery.parallelism` or per-target PDU rate overwhelms a device's SNMP daemon | `credentials.trial_rate_per_second` limits the auth-failure rate. **Gap today** — no per-device steady-state PDU rate limit. Tracked at #72 |
